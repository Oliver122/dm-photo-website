...
- [x] Positive HTTP: public pages return 200 (`/`, `/login`, `/admin/login`).
- [x] Negative HTTP: protected APIs without session → redirect `/login` or HTMX `401`.
- [x] Negative HTTP: admin routes without admin session → redirect `/admin/login` or HTMX `403`.
- [x] Positive HTTP: admin login with correct password unlocks `/admin` (200).
...
| ST-008-c | + | `GET /admin/login` → 200 | `src/system_tests.rs` |
...
| ST-008-f | − | `GET /admin` → redirect `/admin/login` | `src/system_tests.rs` |
| ST-008-g | − | `GET /api/users` + HTMX → 403 | `src/system_tests.rs` |
| ST-008-h | + | Admin login correct → `GET /admin` 200 | `src/system_tests.rs` |
| ST-008-i | − | Admin login wrong → still not admin | `src/system_tests.rs` |
