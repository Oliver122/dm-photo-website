...
| `GET /login` | public | `login.html` | Discord login; `?denied=1` → German “not allowlisted” error |
| `GET /gear` | user | `gear.html` | Camera + lens library |
| `GET /admin/login` | user | `admin_login.html` | Admin password form (Discord session required; anonymous → `/login`) |
| `GET /admin` | admin | `admin.html` | Ticket tools + allowlist CRUD + user table; banner when default admin password |
...
| `POST /admin/login` | user | form → redirect | `password` (Discord session required) |
...
| Admin | Discord session + (`ADMIN_PASSWORD` flag or admin Discord ID) | Extra `/admin` + `/api/users` + allowlist CRUD |
...
| `#allowlist-list` | admin | allowlist CRUD | List swap target (`hx-target`) |
| `#allowlist-out` | admin | allowlist CRUD | OOB feedback (`.notice` / `.error`) |
...
| `POST` | `/admin/allowlist/:key/admin` | — | Toggle admin flag (claimed snowflake **or** pending handle) → `#allowlist-list` |
| `DELETE` | `/admin/allowlist/:key` | — | Remove entry (refuse if last admin across claimed + pending); list + OOB `#allowlist-out` |

Admin login form: `password` → `POST /admin/login` (requires Discord session first; anonymous password → redirect `/login`).

**Admin allowlist panel** (`admin.html`): form (`identity` = Discord username handle or snowflake ID, optional `is_admin` checkbox) → `POST /admin/allowlist` with `hx-target="#allowlist-list"`; username-only rows are **pending** (badge `ausstehend`) until first OAuth binds them to a snowflake; toggle/delete path param is `member.key` (snowflake or handle); OOB feedback → `#allowlist-out`. Warning banner when `default_admin_password` is true.
...
| `is_admin` | session + allowlist | Show Admin nav (password flag or allowlist `is_admin`) |
| `default_admin_password` | env `ADMIN_PASSWORD == changeme` | Admin dashboard warning banner |
